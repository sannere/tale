...
## JavaScript 全局对象
1)  全局属性和函数可用于所有内建的 JavaScript 对象。
2)  全局对象是预定义的对象，作为 JavaScript 的全局函数和全局属性的占位符。
3)  在顶层 JavaScript 代码中，可以用关键字 this 引用全局对象。1. 因为全局对象是作用域链的头，这意味着所有非限定性的变量和函数名都会作为该对象的属性来查询。在顶层 JavaScript 代码中声明的所有变量都将成为全局对象的属性。
4)  全局对象只是一个对象，而不是类。既没有构造函数，也无法实例化一个新的全局对象。
5)  在客户端 JavaScript 中，全局对象就是 Window 对象，表示允许 JavaScript 代码的 Web 浏览器窗口。

## 执行环境
1)  执行环境始终是this关键字的值，它是拥有当前所执行代码的对象的引用。每个执行环境都有一个与之关联的变量对象，环境中定义的所有变量和函数都保存在这个对象中。


## 函数的作用域,this的作用域
1)  在JavaScript中，this指向当前的上下文，而var定义的变量值在当前作用域中有效。JavaScript有两种作用域，全局作用域和局部作用域。局部作用域就是在一个函数里。var关键字使用来在当前作用于中创建局部变量的，**而在浏览器中的JavaScript全局作用域中使用var语句时，会把申明的变量挂在window上**，而全局作用域中的this上下文恰好指向的又是window，因此在全局作用域中var申明的变量和window上挂的变量，即this可访问的变量有间接的联系，但没有直接联系，更不是一样的。
2)  全局环境的this会指向全局对象window，此时this===window;
3)  全局变量会挂载在window对象下，会成为window下的一个属性。
4)  如果你没有使用严格模式并给一个未声明的变量赋值的话，JS会自动创建一个全局变量。
5)  **this的指向，是在函数被调用的时候确定的**。
6)  如果调用者函数，被某一个对象所拥有，那么该函数在调用时，内部的this指向该对象。如果函数独立调用，那么该函数内部的this，则指向undefined。但是在非严格模式中，当this指向undefined时，它会被自动指向全局对象。

## 挂载
1)  将组件渲染，并且构造 DOM 元素然后塞入页面的过程称为组件的挂载。

## axios
1)  它一个网络请求库，它以基于Promise的方式封装了浏览器的XMLHttpRequest和服务器端node http请求，使得我们可以用es6推荐的异步方式处理网络请求。
2)  

## .then().catch()
1)  then()方法是异步执行。意思是：就是当.then()前的方法执行完后再执行then()内部的程序，这样就避免了，数据没获取到等的问题；.catch()抛出异常。
2)  语法：promise.then(onCompleted, onRejected);.catch(onRejected)
3)  参数：promise：必需。Promise 对象;onCompleted：必需。承诺成功完成时要运行的履行处理程序函数。
